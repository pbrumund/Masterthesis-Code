...
def generate_features(weather_data, time, n_last=3, feature='error', steps_ahead = 1):
    ...
    for i in range(n_last):
        ...
        wind_predictions[i] = utils.get_NWP(weather_data, start_time, i)
    ...
    if feature == 'error':
        ...
        return np.concatenate([measurements-wind_predictions, [np.sqrt(cape), temperature, steps_ahead]])
    elif feature == 'measurement & nwp':
        return np.concatenate([measurements, wind_predictions, [np.sqrt(cape), temperature]])
    else:
...
def generate_labels(weather_data, time, label='error', steps_ahead = 1):
    ...
    if label == 'error':
        ...
        previous_measurement = utils.get_wind_value(weather_data, time, steps_ahead=0)
        return measurement-previous_measurement
    else:
        raise ValueError("Unknown value for feature")
...
def multi_step_prediction_time_input(weather_data, time, steps):
    ...
    gp = hilo.GaussianProcess(features=feature_names,
                                labels=label,
                                kernel=kernel1)
    X_train, y_train = sample_random_inputs(weather_data, 100, 3, 'error & nwp', 'error', steps_ahead=steps_ahead)        
    gp.set_training_data(X_train, y_train)
    gp.setup()
    gp.fit_model()
    for i in range(steps):
...
if __name__ == "__main__":
    ...
    n_samples = 100
    n_features = 3
    input_feature = 'measurement & nwp'
    label = 'error'
    steps_ahead = 10

    weather_data = load_weather_data(start_time, end_time)

    X_train, y_train = sample_random_inputs(weather_data, n_samples, n_features, input_feature, label, steps_ahead=steps_ahead)

    n_inputs = X_train.shape[0]
    feature_names = [f"{input_feature}_{i}" for i in range(n_inputs)]

    kernel1 = hilo.Matern52Kernel(active_dims=np.arange(n_inputs), ard=True)
    kernel2 = hilo.Matern52Kernel(active_dims=np.arange(n_inputs), ard=False)
    kernel3 = hilo.LinearKernel()
    gp = hilo.GaussianProcess(features=feature_names,
                              labels=label,
                              kernel=kernel1)
    ...
    for i in range(n_features, evaluation_horizon):
        ...
        features_i = generate_features(weather_data, time=t, n_last=n_features, feature=input_feature, steps_ahead=steps_ahead)
