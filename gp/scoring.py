...
def get_trajectory_nwp(weather_data, opt):
    ...
    meas_traj = np.zeros(n_steps)
    for i, t in enumerate(times):
        meas_traj[i] = utils.get_NWP(weather_data, t, 0)
    return meas_traj
...
def get_posterior_trajectories(opt):
    try:
        trajectories_mean = np.loadtxt('gp/scoring/trajectories_mean_post.csv')
        trajectories_var = np.loadtxt('gp/scoring/trajectories_var_post.csv')
    ...
    for i, time in enumerate(times):
        ...
        with open('gp/scoring/trajectories_mean_post.csv', 'a') as file:
            np.savetxt(file, trajectory_mean_gp.reshape((1,-1)))
        with open('gp/scoring/trajectories_var_post.csv', 'a') as file:
...
if __name__ == '__main__':
    ...
    try:
        trajectory_measured = np.loadtxt('gp/scoring/trajectory_meas.csv')
    except:
        trajectory_measured = get_trajectory_measured(weather_data, opt)
        np.savetxt('gp/scoring/trajectory_meas.csv', trajectory_measured)
    try:
        trajectory_nwp = np.loadtxt('gp/scoring/trajectory_nwp.csv')
    except:
        trajectory_nwp = get_trajectory_nwp(weather_data, opt)
        np.savetxt('gp/scoring/trajectory_nwp.csv', trajectory_nwp)
    try:
        trajectory_gp_prior = np.loadtxt('gp/scoring/trajectory_gp_prior.csv')
        var_gp_prior = np.loadtxt('gp/scoring/var_gp_prior.csv')
    except:
        trajectory_gp_prior, var_gp_prior = get_trajectory_gp_prior(weather_data, opt)
        np.savetxt('gp/scoring/trajectory_gp_prior.csv', trajectory_gp_prior)
        np.savetxt('gp/scoring/var_gp_prior.csv', var_gp_prior)
