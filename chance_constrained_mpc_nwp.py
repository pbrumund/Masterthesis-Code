...
mpc_opt = get_mpc_opt(N=36, use_soft_constraints_state=False, epsilon_chance_constraint=0.001)#, t_start=datetime.datetime(2022,12,6), t_end=datetime.datetime(2022,12,8))
...
data_saver = DataSaving('chance_constrained_mpc_nwp_forecast_with_llc', mpc_opt, gp_opt, dims)
...
for k, t in enumerate(times, start=start):
    ...
    wind_speeds = [data_handler.get_measurement(t, i) for i in range(chance_constrained_mpc.horizon)]
    wind_speeds_nwp[0] = wind_speeds[0]
    train = (k==start) or (t.minute==0)
    std_gp = 1.5656*ca.DM.ones(chance_constrained_mpc.horizon)
    std_gp[0] = 0
    var_gp = std_gp**2
    wind_speeds_gp = wind_speeds_nwp
