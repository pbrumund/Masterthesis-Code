...
def get_gp(weather_data, opt):
    ...
    # X_train, y_train = utils.sparsify_training_data(X_train, y_train, 10000)
    ...
    gp = SparseGP(X_train, y_train, likelihood, opt)
...
def predict_uncertain_inputs(gp, likelihood, x, input_cov, opt):
...
def predict_trajectory(weather_data, t_start, steps_ahead, gp, likelihood, opt):
    ...
    for i in range(steps_ahead):
        ...
        mean, var, cov = predict_uncertain_inputs(gp, likelihood, x, input_cov, opt)
...
if __name__ == "__main__":
    ...
    gp, likelihood = get_gp(weather_data, opt)
    ...
    gp_traj, var_traj = predict_trajectory(weather_data, t, steps_ahead, gp, likelihood, opt)
