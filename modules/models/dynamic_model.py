...
class DynamicModel():
    ...
    def outfun(self, x=None, u=None, p=None, w = None):
        outfun = self._outfun
        if x is None:
            return self._outfun
    ...
    @property
    def lbx(self):
        lbx = self._bounds.get('lbx')
        if lbx is None:
            lbx = -ca.inf*ca.DM.ones(self._nx)
        return lbx
    @property
    def ubx(self):
        ubx = self._bounds.get('ubx')
        if ubx is None:
            ubx = ca.inf*ca.DM.ones(self._nx)
        return ubx
    @property
    def lbu(self):
        lbu = self._bounds.get('lbu')
        if lbu is None:
            lbu = -ca.inf*ca.DM.ones(self._nu)
        return lbu
    @property
    def ubu(self):
        ubu = self._bounds.get('ubu')
        if ubu is None:
            ubu = ca.inf*ca.DM.ones(self._nu)
        return ubu
    ...
    def x0(self, x_0):
        if x_0.shape != self._x.shape:
            raise ValueError('Initial condition does not match state dimension')
        self._x0 = x_0
    @property
